...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        if self.crossword.overlaps[x, y]:
            ...
            for x_val in self.domains[x].copy():
                found_corresponding_val = False
                for y_val in self.domains[y]:
                    if x_val[i] == y_val[j]:
                        found_corresponding_val = True
                        break
                if not found_corresponding_val:
        ...
        for var in assignment:
            ...
            for neighbour in self.crossword.neighbors(var):
                if neighbour in assignment:
                    neighbour_val = assignment[neighbour]
                    if self.crossword.overlaps[var, neighbour]:
    ...
    def order_domain_values(self, var, assignment):
        ...
        """
        rules_out = {val: 0 for val in self.domains[var]}
        ...
        sorted([val for val in self.domains[var]], key=lambda val: rules_out[val])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        return values.
        """
        available_vars = self.domains.keys() - assignment.keys()
        available_vars = list(available_vars)
        min_no_values = available_vars[0]
        sorted(available_vars, key=lambda var: len(self.crossword.neighbors(var)), reverse=True)
        for i in range(1, len(available_vars)):
            if len(self.domains[available_vars[i]]) < len(self.domains[min_no_values]):
                min_no_values = available_vars[i]

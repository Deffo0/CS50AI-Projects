...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                if (i, j) not in self.moves_made and (0 <= i < self.width) \
                        and (0 <= j < self.height) and (i, j) not in self.mines and (i, j) not in self.safes:
        ...
        for sentence_A in self.knowledge.copy():
            for sentence_B in self.knowledge.copy():
                if (sentence_A.cells.issubset(sentence_B.cells)) and (sentence_A.cells != sentence_B.cells):
                    inference = Sentence(sentence_B.cells - sentence_A.cells,
